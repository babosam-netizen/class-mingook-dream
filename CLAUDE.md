...
| `<배포경로>/app/` | **배포 경로**(`https://<배포주소>/class_democra/app/`). 로컬에서 `npm run build`한 `dist/` 산출물만 복사 |
| `<배포경로>/uploads/` | **레거시 이미지 보관 폴더**. 과거 업로드된 포스터·증거 사진 표시용으로만 유지(신규 업로드 없음). ⚠️ 삭제 금지 |
| ~~`업로드엔드포인트`~~ | **제거됨**(v1.7.33). 이미지 업로드 기능 폐지 — 포스터·증거는 Canva 임베드/이미지 URL로 처리 |
| `<배포경로>/fetch_meta.php` | 링크 미리보기 메타 추출 PHP(선택). 실패 시 microlink로 자동 fallback |
...
- 호스팅: 서버 정적 파일(`https://<배포주소>/class_democra/app/`) / 포크본은 Cloudflare Pages 등 정적 호스팅
