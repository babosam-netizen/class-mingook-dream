...
export function normalizeRoleList(kind, roles) {
  ...
  // 대통령실 역할 세트(pledge_*)는 일반 부처 4역할(minister/planner/…)과 키가 완전히 다르다.
  // 아래 일반 executive 기본역할 강제 정렬을 그대로 적용하면 대통령 역할이 extras로 밀리고,
  // BranchUnitWorkspace의 slice(0, memberCount)에서 잘려 대표 역할(pledge_purpose)을 고를 수 없게 된다.
  // → 대통령 역할 세트는 정규화만 하고 원래 순서(대표가 맨 앞)를 그대로 보존한다.
  const presidentKeys = new Set((DEFAULT_ROLES.executive_president || []).map((role) => role.key))
  if (normalized.length > 0 && normalized.some((role) => presidentKeys.has(role.key))) {
    return normalized
  }
