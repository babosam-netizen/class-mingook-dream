...
## v1.7.49 (2026-06-19) [Claude] — 교사 기사 관리에서 본문 전체 펼쳐보기
- 교사 토론도구 토론 후 탭의 기사 관리(`ArticleApprovalQueue`)에서 학생 기사 본문이 `line-clamp-3`로 잘려 전체를 읽기 어렵던 것 → **본문 클릭 시 전체 펼침/접기**(80자 초과 시 ‘전체 보기/접기’ 표시). 교사는 기존처럼 [대기/승인/신고/삭제/전체] 필터로 학생이 쓴 기사 내용을 모두 확인 가능.
- `APP_BUILD` v1.7.49.

## v1.7.48 (2026-06-19) [Claude] — 최종 종합평가를 참고 메모 바로 아래로 이동
- 판결중심 토론도구에서 `EvaluatorFinalCommentForm`(최종 종합평가)이 화면 맨 아래에 있던 것을 **참고 메모 바로 아래**로 이동.
- 참관 판사 뷰: 참관 판사 메모(참고용) → 최종 종합평가 → 평가 단계. 연기팀(일반) 뷰: 판결문 참고 메모 → 최종 종합평가 → 판결문 작성. 양 뷰 각각의 메모 아래에 배치하고, 기존 mid탭 하단 공용 블록은 제거(중복 방지). 토론 후 탭은 기존 유지.
- `APP_BUILD` v1.7.48.
...
## v1.7.44 (2026-06-19) [Claude] — 판결중심 4종: 법·시행령 뷰어, 판결문 작성자 선정, 초안 검토, 제출 체크리스트
- **① 법·시행령 뷰어(`LawDecreeButton`)**: 사건 자료실 옆 알약형 버튼. 통과 법안(법)+행정부 시행령을 모달로 표시 — 구형·벌금이 벌칙 범위를 넘는지 확인용. 판결중심 탭(쟁점·재판하기)·토론도구에 배치.
- **②(a) 모둠 판결문 작성자 선정**: `verdictDrafts/{gid}.writerStudentId`. 한 명이 [내가 작성자 맡기]로 대표가 되면 나머지는 **읽기 모드(실시간 표시)**, 작성자 본인·교사만 편집/넘기기/초기화. 비작성자 로컬상태는 draft와 동기화해 인수인계 시 최신본 유지.
- **②(b)·③(답글)**: 검사·변호·판사(연기팀)도 토의 단계에서 다른 모둠 판결문 평가 가능(단계 게이팅이라 이미 됨), 우리 모둠 판결문엔 모둠원이 답글 가능(`CommentList` `targetGroupId`로 이미 됨) — 확인만, 변경 없음.
- **③ 교사 초안 검토**: `Phase3JudicialQuickPanel`의 `JudicialContentReview`에 토론 세션 `verdictDrafts`(작성 중 초안)를 추가해 제출 전 내용을 간략→펼침으로 검토.
- **④ 제출 체크리스트**: 모둠 판결문 게시 시 3개 항목(벌칙 범위 / 결론-이유 일치 / 증거 근거) 모두 체크해야 게시.
- `npm run build` 통과 + dev 부팅 스모크. `APP_BUILD` v1.7.44.
